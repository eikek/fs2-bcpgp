...
object EncryptSpec extends SimpleTestSuite with FileFixtures with Imports.Defaults {
  val chunkSize = 8192
  ...
  def asString: Pipe[IO, Byte, String] =
    _.through(text.utf8Decode).chunks.map(_.toVector.mkString).fold1(_ + _)
  ...
  test("pubkey encrypt stream of larger file") {
    ...
    def encryptFile(from: Path, to: Path) =
      io.file
        .readAll[IO](from, blocker, 96 * 1024)
        .through(encrypt.pubkey[IO](key, chunkSize, blocker))
  ...
  test("encrypt stream symmetric") {
    ...
    val org = in
      .through(encrypt.symmetric(SymmetricAlgo.AES256, pass, chunkSize, blocker))
  ...
  test("pk encrypt java.io") {
    ...
    val org = Stream
      .chunk(Chunk.bytes(out.toByteArray))
      .covary[IO]
      .through(decrypt.pubkey[IO](ks, _ => "test".toCharArray, blocker))
      .through(asString)
      .compile
