...
object syntax {
  ...
  private def pipeStreams[F[_], A, B](
      f: (InputStream, OutputStream) => F[Unit],
      to: A => Array[Byte],
      from: Array[Byte] => B
  ...
  implicit class StringCrypt(s: String) {
    private def encryptStreams[F[_]](
        f: (InputStream, OutputStream) => F[Unit],
        charset: Charset
    )(implicit F: Sync[F]): F[String] =
      pipeStreams[F, String, String](
        f,
        s => s.getBytes(charset),
        ba => ByteVector.view(ba).toBase64
      ).apply(s)

    private def decryptStreams[F[_]](
        f: (InputStream, OutputStream) => F[Unit],
        charset: Charset
    )(implicit F: Sync[F]): F[String] =
      pipeStreams[F, String, String](
        f,
        s => ByteVector.fromValidBase64(s).toArray,
        ba => new String(ba, charset)
      ).apply(s)

    def encryptPubkey[F[_]](key: Pubkey, charset: Charset = StandardCharsets.UTF_8)(
        implicit F: Sync[F]
    ): F[String] =
      encryptStreams(encrypt.pubkeySync(_, key, _), charset)

    def encryptSymmetric[F[_]](
        pass: Array[Char],
        algo: SymmetricAlgo = SymmetricAlgo.default,
  ...
  implicit class ByteVectorCrypt(bv: ByteVector) {
    ...
    def decryptPubkey[F[_]](ks: Keystore, pass: Long => Array[Char])(
        implicit F: Sync[F]
    ): F[ByteVector] =
      pipeStreams[F, ByteVector, ByteVector](
