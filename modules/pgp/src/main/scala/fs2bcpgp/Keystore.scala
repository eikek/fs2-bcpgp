...
case class Keystore(public: PublicKeys, secret: SecretKeys) {
  ...
  def addNewKeypair[F[_]](userID: String, pass: Array[Char], strength: Int = 4096)(
      implicit F: Sync[F]
  ): F[Keystore] =
    Keystore.generate[F](userID, pass, strength).map(ks => this ++ ks)

  def ++(other: Keystore): Keystore =
    Keystore(public ++ other.public, secret ++ other.secret)

  def checkPassword[F[_]](
      pw: Long => Array[Char]
  )(implicit F: Sync[F]): F[List[(Long, Boolean)]] =
...
object Keystore {
  ...
  def generate[F[_]: Sync](
    ...
    signhashgen.setKeyFlags(
      false,
      KeyFlags.SIGN_DATA | KeyFlags.CERTIFY_OTHER | KeyFlags.ENCRYPT_COMMS | KeyFlags.ENCRYPT_STORAGE
    )
    ...
    val keyRingGen = new PGPKeyRingGenerator(
      ...
      new BcPGPContentSignerBuilder(
        pubkey.getPublicKey().getAlgorithm(),
        HashAlgorithmTags.SHA1
      ),
      pske
    )
  ...
  def fromInputStream[F[_]: Sync: ContextShift](
      in: F[InputStream],
      blocker: Blocker,
    ...
    io.readInputStream[F](in, 8192, blocker, closeAfterUse)
      ...
      .compile
      .toVector
      .flatMap(fromArmoredLines[F])

  def fromArmoredFile[F[_]](f: Path)(implicit F: Sync[F]): F[Keystore] =
    F.delay(Files.readAllLines(f).toScala).flatMap(lines => fromArmoredLines(lines))
  ...
  private def fromArmoredLines[F[_]: Sync](lines: Iterable[String]): F[Keystore] = {
    val (pub, sec, _, _) = lines.foldLeft(("", "", false, false)) {
      case ((publines, seclines, pub, sec), line) =>
        line match {
  ...
  private[fs2bcpgp] def toArmored[F[_]](bytes: ByteVector)(implicit F: Sync[F]) =
    F.delay {
  ...
  private[fs2bcpgp] def fromArmored[F[_]](
      armored: String
