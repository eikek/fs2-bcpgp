...
object decrypt {
  ...
  def pubkeySync[F[_]](in: InputStream, ks: Keystore, pass: Long => Array[Char], out: OutputStream)(implicit F: Sync[F]): F[Unit] = {
    ...
    val dec: F[Iterator[F[Unit]]] = F.delay {
      makeEncData[PGPPublicKeyEncryptedData](in).
        map({ pgpobj =>
          getKey(pgpobj.getKeyID).map(pk =>
            pgpobj.getDataStream(new JcePublicKeyDataDecryptorFactoryBuilder()
  ...
  def symmetricSync[F[_]](in: InputStream, pass: Array[Char], out: OutputStream)(implicit F: Sync[F]): F[Unit] = {
    val dec: F[Iterator[Unit]] = F.delay {
      makeEncData[PGPPBEEncryptedData](in).
        map({ pgpobj =>
          unwrapPGP(pgpobj.getDataStream(new JcePBEDataDecryptorFactoryBuilder(
    ...
    val clearText: Stream[F, InputStream] = in.
      ...
      flatMap { pgpobj =>
        Stream.eval(ks.secret.find(pgpobj.getKeyID)).flatMap {
          case Some(k) => Stream.eval(k.extractPrivateKey(pass(pgpobj.getKeyID))).map(privKey =>
            pgpobj.getDataStream(new JcePublicKeyDataDecryptorFactoryBuilder()
              .setProvider(Provider.name)
              .build(privKey.key)))
          case None => Stream.raiseError(new Exception(s"No private key found for id: ${pgpobj.getKeyID}"))
        }
      }
